...
# Добавьте эти функции в начале файла после объявления States

# Функция для получения и инициализации состояния пользователя
async def get_user_state(user_id, chat_id):
    """Получает текущее состояние пользователя с учетом типа чата"""
    is_group = chat_id < 0  # Отрицательный ID означает групповой чат
    
    if is_group:
        # Инициализация для группового чата
        ...
        return group_states[chat_id][user_id]
    else:
        # Инициализация для личного чата
...
async def set_user_state(user_id, state, chat_id):
    """Устанавливает состояние пользователя с учетом типа чата"""
    is_group = chat_id < 0
    
    if is_group:
        if chat_id not in group_states:
            group_states[chat_id] = {}
        group_states[chat_id][user_id] = state
        logger.info(f"Установлено состояние {state} для пользователя {user_id} в групповом чате {chat_id}")
    else:
        user_states[user_id] = state
        logger.info(f"Установлено состояние {state} для пользователя {user_id} в личном чате")

# Функция для получения контекста пользователя
async def get_user_context(user_id, chat_id, key=None, default=None):
    """Получает контекст пользователя с учетом типа чата"""
    is_group = chat_id < 0
    ...
    if is_group:
        ...
        context = group_contexts[chat_id][user_id]
    else:
    ...
    # Возвращаем значение конкретного ключа или весь контекст
    if key is not None:
        return context.get(key, default)
    return context

# Функция для обновления контекста пользователя
async def set_user_context(user_id, chat_id, key, value):
    """Устанавливает значение в контексте пользователя"""
    is_group = chat_id < 0
    ...
    if is_group:
        ...
        group_contexts[chat_id][user_id][key] = value
    else:
        if user_id not in user_contexts:
            user_contexts[user_id] = {}
...
# Функция для сброса состояния и контекста пользователя
async def reset_user_state_and_context(user_id, chat_id):
    """Сбрасывает состояние и контекст пользователя"""
    is_group = chat_id < 0
    
    if is_group:
        if chat_id in group_states and user_id in group_states[chat_id]:
            group_states[chat_id][user_id] = States.IDLE
        if chat_id in group_contexts and user_id in group_contexts[chat_id]:
...
async def initialize_user_data(user_id, chat_id):
    """Инициализирует данные пользователя при необходимости"""
    is_group = chat_id < 0
...
async def process_message(message):
    try:
        ...
        await initialize_user_data(user_id, chat_id)
        ...
        current_state = await get_user_state(user_id, chat_id)
        ...
        if current_state == States.WAITING_DEED:
            ...
            await set_user_context(user_id, chat_id, 'deed_description', text)

            # Просим ввести количество баллов
            username = message['from']['username'] if 'username' in message['from'] else 'пользователь'
            msg = f"👍 @{username}, опиши, сколько баллов ты получил за это дело:"
            if not is_group:
                msg = f"👍 Отлично! Теперь укажи, сколько баллов ты получил за это дело:"
    
            await send_message(chat_id, msg)

            # Обновляем состояние пользователя
            await set_user_state(user_id, States.WAITING_POINTS, chat_id)
            ...
            try:
                ...
                deed_description = await get_user_context(user_id, chat_id, 'deed_description', 'Доброе дело')
                logger.info(f"Получено описание дела из контекста: {deed_description}")
                ...
                await reset_user_state_and_context(user_id, chat_id)
        ...
        # Обработка ввода пароля родителя
            ...
            password = text
            is_registering = await get_user_context(user_id, chat_id, 'registering_parent', False)
            ...
            if is_registering:
                ...
                await set_user_state(user_id, States.PARENT_MENU, chat_id)
            else:
                ...
                try:
                    ...
                    await send_message(
                        chat_id,
                        "Произошла ошибка. Возможно, вы еще не зарегистрированы как родитель. Попробуйте еще раз."
                    )
        ...
        # Обработка ввода описания доброго дела от родителя
            ...
            # Обновляем состояние
            await set_user_state(user_id, States.PARENT_ADD_POINTS, chat_id)
            return
        ...
        if text == '/start':
            ...
            await set_user_state(user_id, States.IDLE, chat_id)
        ...
        # Обработка команды /add
            ...
            await set_user_state(user_id, States.WAITING_DEED, chat_id)
...
async def process_callback_query(callback_query):
    try:
        ...
        if data == 'add_deed':
            ...
            username = callback_query['from'].get('username', callback_query['from'].get('first_name', 'пользователь'))
            ...
            # Устанавливаем состояние
            await set_user_state(user_id, States.WAITING_DEED, chat_id)
            logger.info(f"Установлено состояние WAITING_DEED для пользователя {user_id} в чате {chat_id}")
        ...
        # Обработка кнопки "Я родитель"
            ...
            await set_user_state(user_id, States.PARENT_PASSWORD, chat_id)
            await set_user_context(user_id, chat_id, 'registering_parent', True)
        ...
        # Обработка кнопки "Добавить ребенка"
            ...
            await set_user_state(user_id, States.PARENT_ADD_CHILD, chat_id)
        ...
        # Обработка выбора ребенка (формат: select_child_TELEGRAM_ID)
            ...
            try:
                ...
                await set_user_state(user_id, States.PARENT_ADD_DEED, chat_id)
                await set_user_context(user_id, "selected_child_id", child_telegram_id)
...
async def process_parent_command(chat_id, user_id):
    ...
    try:
        ...
        if is_parent:
            ...
            await set_user_state(user_id, States.PARENT_PASSWORD, chat_id)
            await set_user_context(user_id, chat_id, 'registering_parent', False)
        else:
            await send_message(
                chat_id,
